...
#FORMAT DATA
cat ghost.info.html | sed '/^$/d' | sed 's/^/" /g' | sed 's/$/",/g' | sed '1i { "info" : [ ' | sed '$ s/.$/] , /' > $leChemin"/data/dailyData.json"
...
echo '"gooSearch" : '$GOOFR $GOOIE $GOOGB $GOOBE $GOOUS >> $leChemin"/data/dailyData.json"
...
echo '"citation" : '$CITATION >> $leChemin"/data/dailyData.json"
echo ' CITATION _ OK'
...
echo '"cinema" : ['$CINEIMG'],' >> $leChemin"/data/dailyData.json"
echo ' CINEMA _ OK'
...
echo '"futurCinema" : ['$CINEFUTUR'], "futurCinemaIMG" : ['$CINEFUTURIMG'],' >> $leChemin"/data/dailyData.json"
...
echo '"films" : ['$FILM'], "filmsIMG" : ['$FILMIMG'],' >> $leChemin"/data/dailyData.json"
echo ' FILM TOP _ OK'
...
echo '"series" : ['$SERIE'] , "seriesIMG" : ['$SERIEIMG'],' >> $leChemin"/data/dailyData.json"
echo ' SERIE TOP _ OK'
...
echo '"ministereKult" : [ '$MINISTREKULTURE' ] , "mondeArt" : ['$ART'],' >> $leChemin"/data/dailyData.json"
echo ' CULTURE _ OK'
...
echo '"photo" : ['$TOF >> $leChemin"/data/dailyData.json"
echo ' PHOTO _ OK'
...
echo '"sante" : { "ars" : [ '$ARS $OBS '] , "sentinl" : [' $SENTINEL" } ," >> $leChemin"/data/dailyData.json"
echo ' SANTE _ OK'
...
# echo '"gouv" : '$GOUV >> $leChemin"/data/dailyData.json"
echo '"gouv" : [ "", "" ] ,' >> $leChemin"/data/dailyData.json"
echo ' GOUV _ OK'

# LE MONDE
UNE=$(curl -s https://www.lemonde.fr/rss/une.xml | sed 's/</\n/g' | grep CDATA | sed 's/"//' | head -n 10 | sed 's/!\[CDATA\[\|\]\]>/"/g' | sed 's/$/,/g' | sed '$ s/.$/] ,/g' | sed '1s/^/[/g' )
echo '"une" : '$UNE >> $leChemin"/data/dailyData.json"
echo ' LE MONDE _ OK'
...
echo '"tech" : ['$ANSSI_ACTU$TECH >> $leChemin"/data/dailyData.json"
echo ' TECH _ OK'

# SECURITE INFO
CONSULTANT=$(curl -s https://www.dsfc.net/feed/ | grep '<title' | sed 's/title>/"/g' | sed 's/<\|\t//g' | sed 's/\/"/",/g' |  head -n 6 | sed '$ s/.$//' | sed 1d )
echo '"consultantCyber" : ['$CONSULTANT'],' >> $leChemin"/data/dailyData.json"
...
# echo '"korben" : [ '$KORBEN '],' >> $leChemin"/data/dailyData.json"
# echo ' KORBEN _ OK'
...
echo '"newsaudio" : "'$INFOAUDIO'",' >> $leChemin"/data/dailyData.json"
echo "AUDIO OK"
...
#FORMAT DATA
head -n 50 ghost.twitter.buffer | awk '!a[$0]++' | sed 's/$/",/' | sed 's/^/"/' | sed '1i "twitter" : [ " ",  " 🇫🇷 " ," ", ' | sed '$ s/.$/ , /' >> $leChemin"/data/dailyData.json"
...
cat ghost.twitter.html | sed 's/https/\n/g' | grep -o 'twitter.com/search?q=.*/a>' | sed 's/\(<\/a>\|">\|\/\)/\n/g' | sed 's/=\|twitter/\nsearch/g' | grep -v 'search' | sed -r '/^\s*$/d' > ghost.twitter.buffer
#FORMAT DATA
head -n 50 ghost.twitter.buffer | awk '!a[$0]++' | sed 's/$/",/' | sed 's/^/"/' | sed '1i " ", " 🌎 "," ", ' |  sed '$ s/.$/ , /' >> $leChemin"/data/dailyData.json"
...
cat $leChemin"/data/dailyData.json" | jq empty
